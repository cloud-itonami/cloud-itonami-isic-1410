(ns apparel.operation
  "ApparelOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (ApparelAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Apparel
  ...
  each operation is auditable and checkpointed. A garment plant's
  lifecycle is advanced by MANY ops (order intake / pattern-spec
  metadata / quality flag / shipment coordination), each independent.
...
(defn build
  "Compiles an ApparelOperationActor graph bound to `store`.
  opts:
    :advisor      -- an `apparel.advisor/Advisor` (default: mock-advisor)
